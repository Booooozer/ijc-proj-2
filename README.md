...
2) (max 10b)
   ...
   Podmínky:
    ...
    - Stručný popis základních funkcí:
		...
		(tuto funkci cvičně použijte v programu překladem `#ifdef MOVETEST`)
        ...
        `b=htab_erase(t,key)`
        zrušení záznamu se zadaným klíčem (úspěch: `true`)

        `htab_for_each(t,funkce)`
        projde všechny záznamy, na každý zavolá funkci (pozor na možné změny tabulky!)
